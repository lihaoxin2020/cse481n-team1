...
counter_path = './datasets/substitution-sets/MRQANaturalQuestionsTraincorpus-substitution.jsonl'
orig_path = './datasets/normalized/MRQANaturalQuestionsTrain.jsonl.gz'
...
temp = list(zip(original_examples, counter_examples))
random.seed(42)
random.shuffle(temp)
original_examples, counter_examples = zip(*temp)
original_examples, counter_examples = list(original_examples), list(counter_examples)
...
added_instances = set()
engine = Engine("alpaca-native")  # specify which model you want to use here by
...
for oe, ce in tqdm(zip(original_examples, counter_examples), total=len(original_examples)):
    ...
    assert len(filtered_counter_examples) == len(filtered_orig_examples)
    print('Get {} filtered examples.'.format(len(filtered_orig_examples)))

    if len(filtered_counter_examples) == 16:
        break

out_counter_path = f'./conflict_train_filtered_{engine.engine}.json'
out_orig_path = f'./orig_train_filtered_{engine.engine}.json'
